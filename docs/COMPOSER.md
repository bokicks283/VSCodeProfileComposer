...
`global/settings.jsonc` contains `workbench.settings.applyToAllProfiles` and exactly one value for every listed setting. The list is derived ownership metadata: generated application settings include every other top-level key exactly once. Repository validation rejects duplicates, missing values, unlisted values, and declarations of globally owned settings in components, profile overrides, or platform overlays.
...
Override paths and source ownership are tracked in memory while composing so
validation and diagnostics remain precise. They are not emitted as separate
end-user files.

## Generated package

Each `build/profiles/<id>/` directory contains exactly one end-user artifact:

- `<Display-Name>.code-profile` — the finished manual-import profile.

Merged settings, extension IDs, keybindings, overrides, and validation results
exist only during composition and are embedded into or used to validate that
finished artifact.

## `.code-profile` export

The finished profile follows VS Code's `IUserDataProfileTemplate` JSON representation verified against stable VS Code 1.129.1 (commit `8a7abeba6e03ea3af87bfbce9a1b7e48fed567b8`). The outer object has a display `name` plus JSON-encoded string resources:

- `settings`: `{ "settings": "<composed settings JSON>" }`
- `extensions`: an array of `{ "identifier": { "id": "publisher.extension" } }`; versions and local installation state are not embedded
- `keybindings`: `{ "keybindings": "<composed keybindings JSON>", "platform": <number> }`
...
```powershell
pwsh ./scripts/ProfileComposer.ps1 compose python-database -Platform windows -UiStateProfile main
pwsh ./scripts/ProfileComposer.ps1 compose python-database -Platform windows -UiStateFromProfile "C:\private\One-off Layout.code-profile"
pwsh ./scripts/ProfileComposer.ps1 compose python-database -Platform windows -NoUiState
```
...
- every top-level built-in Default/application value after automatically normalizing `workbench.settings.applyToAllProfiles`.
